  ...
  return (
    <div>
      <h2 id="album-heading" data-cy="AlbumHeading">
        <Translate contentKey="familyfunApp.album.home.title">Albums</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="familyfunApp.album.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/album/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="familyfunApp.album.home.createLabel">Create new Album</Translate>
          </Link>
        </div>
      </h2>
        ...
        {albumList && albumList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="familyfunApp.album.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {albumList.map((album, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {album.familyTrees
                      ? album.familyTrees.map((val, j) => (
                          <span key={j}>
                            <Link to={`/family-tree/${val.id}`}>{val.id}</Link>
                            {j === album.familyTrees.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="familyfunApp.album.home.notFound">No Albums found</Translate>
            </div>
          )
        )}
      </div>
